...
function PetDetail({ setPetToEdit }) {
    ...
    const getPet = async () => {
        try {
            /* FETCH */
            // const response = await fetch(`http://localhost:8003/pets/${petId}`)
            ...
            /* AXIOS */
            const response = await axios.get(`http://localhost:8003/pets/${petId}`);
            if (response.status === 200) {
    ...
    const deletePet = async () => {
        try {
            ...
            /* AXIOS */
            const response = await axios.delete(`http://localhost:8003/pets/${petId}`);
